...
onready var playerAnim = $playerCont/playerMeshCont/snakeMesh/AnimationPlayer
...
func _ready():
	...
	hpTexture.value = GVariables.snakeHP
	hpText.text = str(GVariables.snakeHP)
	...
	if is_on_floor():
		...
		if direction != Vector3.ZERO:
			if state == 'run':
				...
				playerAnim.play("run")
			elif state == 'landingJump':
				speed = 20
				playerAnim.play("landingJump")
			elif state == 'attack':
				speed = 5
				playerAnim.playback_speed = 1.5
