...
onready var animPlayer = $playerCont/playerMeshCont/snakeMesh/AnimationPlayer
...
func change_random_pos():
	lookAtPos = posArr[floor(rand_range(0,posArr.size()))]
...
func inst_capsule():
	$deadTimer.stop()
	GVariables.snakeCount += 1
	var c  = capsuleItem.instance()
	add_child(c)
	capsuleCol.disabled = true
	yield(get_tree().create_timer(.8), "timeout")
	tween_capture_move()
	print(GVariables.snakeCount)
